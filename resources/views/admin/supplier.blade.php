...
<div class="wrapper">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
       ...
       <div class="navbar-search-block">
          <form class="form-inline" method="post" action="{{url('carisupplier')}}">
            @csrf
            @method('POST')
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="{{url('supplier')}}" class="nav-link active">
  ...
  @if(Auth::user()->karyawan_level == 'ADMIN')
          ...
          <table class="table table-bordered table-hover table-striped" id="tablesupplier">
            <thead>
            <tr>
              <td>NO</td>
              <td>NAMA SUPPLIER</td>
            ...
            <tbody>
            <?php $no=1; ?>
            @foreach($supplier as $ak)
              @if($ak->count() > 0)
              <tr>
                <td>{{$no++}}</td>
                <td>{{$ak->supplier_nama}}</td>
                <td>{{$ak->supplier_alamat}}</td>
                <td>{{$ak->supplier_hp}}</td>
                <td>
                  <button class="btn btn-info btn-md" data-toggle="modal" data-target="#edit{{$ak->supplier_id}}"><i class="fa fa-edit"></i></button> 
                  <button class="btn btn-danger btn-md" data-toggle="modal" data-target="#hapus{{$ak->supplier_id}}"><i class="fa fa-trash"></i></button>

                  <!-- Modal Edit Admin -->
    <div class="modal" role="dialog" id="edit{{$ak->supplier_id}}" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3>Edit Supplier {{$ak->supplier_nama}}</h3>
            <button class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
              <form method="post" action="{{route('crudsupplier.update', $ak->supplier_id)}}">
                @csrf
                @method('PUT')
                ...
                <input type="text" name="supplier_nama" value="{{$ak->supplier_nama}}" class="form-control" id="nama" placeholder="Nama Supplier..." required>
                </label>
                </div>
                ...
                <textarea name="supplier_alamat" class="form-control" placeholder="Alamat..." required>{{$ak->supplier_alamat}}</textarea>
                </label>
                </div>
                ...
                <input name="supplier_hp" id="hp" value="{{$ak->supplier_hp}}" type="number" class="form-control" placeholder="85348746" required>
                </label>
                </div>
    ...
    <!-- Modal Hapus Admin -->
    <div class="modal" id="hapus{{$ak->supplier_id}}" role="dialog" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3>Hapus Supplier {{$ak->supplier_nama}}</h3>
            <button class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
            <form method="post" action="{{route('crudsupplier.destroy', $ak->supplier_id)}}">
              @csrf
              @method('DELETE')
    ...
    <div class="modal" role="dialog" id="tambah" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
              <form method="post" action="{{route('crudsupplier.store')}}">
                @csrf
                @method('POST')
                <div class="form-group">
                Nama Supplier :
                ...
                <input type="text" name="supplier_nama" class="form-control" id="nama" placeholder="Nama Supplier..." required>
                </label>
                </div>
                ...
                <textarea name="supplier_alamat" class="form-control" placeholder="Alamat..." required></textarea>
                </label>
                </div>
                ...
                <input name="supplier_hp" id="hp" type="number" class="form-control" placeholder="85348746" required>
                </label>
                </div>
