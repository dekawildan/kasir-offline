...
<div class="wrapper">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
       ...
       <div class="navbar-search-block">
          <form class="form-inline" method="post" action="{{url('caricustomer')}}">
            @csrf
            @method('POST')
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <table class="table table-bordered table-hover table-striped" id="tablemember">
            <thead>
            <tr>
              <td>NO</td>
              <td>NAMA</td>
            ...
            <tbody>
            <?php $no=1; ?>
            @foreach($member as $ak)
              @if($ak->count() > 0)
              <tr>
                <td>{{$no++}}</td>
                <td>{{$ak->customer_nama}}</td>
                <td>{{$ak->tgl_daftar}}</td>
                <td>{{$ak->nohp}}</td>
                <td>{{$ak->customer_level}}</td>
                <td>{{$ak->customer_status}}</td>
                <td>
                  <button class="btn btn-info btn-md" data-toggle="modal" data-target="#edit{{$ak->customer_id}}"><i class="fa fa-edit"></i></button> 
                  <button class="btn btn-danger btn-md" data-toggle="modal" data-target="#hapus{{$ak->customer_id}}"><i class="fa fa-trash"></i></button>

                  <!-- Modal Edit Admin -->
    <div class="modal" role="dialog" id="edit{{$ak->customer_id}}" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3>Edit Member {{$ak->customer_nama}}</h3>
            <button class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
              <form method="post" action="{{route('crudcustomer.update', $ak->customer_id)}}">
                @csrf
                @method('PUT')
                ...
                <input type="text" name="customer_nama" value="{{$ak->customer_nama}}" class="form-control" id="nama" placeholder="Nama Member..." required>
                </label>
                </div>
                ...
                <input type="date" name="tgl_daftar" value="{{$ak->tgl_daftar}}" class="form-control" id="tgl" placeholder="Tanggal Daftar..." required>
                </label>
                </div>
                ...
                <input name="nohp" id="hp" value="{{$ak->nohp}}" type="number" class="form-control" placeholder="85348746" required>
                </label>
                </div>
                ...
                <select class="form-control" name="customer_level" id="level">
                  <option selected>{{$ak->customer_level}}</option>
                  <option>UMUM</option>
                ...
                <select class="form-control" name="customer_status" id="jenis">
                  <option selected>{{$ak->customer_status}}</option>
                  <option>AKTIF</option>
    ...
    <!-- Modal Hapus Admin -->
    <div class="modal" id="hapus{{$ak->customer_id}}" role="dialog" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3>Hapus Member {{$ak->customer_nama}}</h3>
            <button class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
            <form method="post" action="{{route('crudcustomer.destroy', $ak->customer_id)}}">
              @csrf
              @method('DELETE')
    ...
    <div class="modal" role="dialog" id="tambah" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
              <form method="post" action="{{route('crudcustomer.store')}}">
                @csrf
                @method('POST')
                <div class="form-group">
                Nama Member :
                ...
                <input type="text" name="customer_nama" class="form-control" id="nama" placeholder="Nama Member..." required>
                </label>
                </div>
                ...
                <input type="date" name="tgl_daftar" class="form-control" id="tgl" placeholder="Tanggal Daftar..." required>
                </label>
                </div>
                ...
                <input name="nohp" id="hp" type="number" class="form-control" placeholder="85348746" required>
                </label>
                </div>
