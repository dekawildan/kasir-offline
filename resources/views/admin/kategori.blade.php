...
<div class="wrapper">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
       ...
       <div class="navbar-search-block">
          <form class="form-inline" method="post" action="{{url('carikategori')}}">
            @csrf
            @method('POST')
  ...
  @if(Auth::user()->karyawan_level == 'ADMIN')
          ...
          <table class="table table-bordered table-hover table-striped" id="tablekategori">
            <thead>
            <tr>
              <td>NO</td>
              <td>NAMA KATEGORI</td>
            ...
            <tbody>
            <?php $no=1; ?>
            @foreach($kategori as $ak)
              @if($ak->count() > 0)
              <tr>
                <td>{{$no++}}</td>
                <td>{{$ak->kategori_nama}}</td>
                <td>{{$ak->kategori_deskripsi}}</td>
                <td>
                  <button class="btn btn-info btn-md" data-toggle="modal" data-target="#edit{{$ak->kategori_id}}"><i class="fa fa-edit"></i></button> 
                  <button class="btn btn-danger btn-md" data-toggle="modal" data-target="#hapus{{$ak->kategori_id}}"><i class="fa fa-trash"></i></button>

                  <!-- Modal Edit Admin -->
    <div class="modal" role="dialog" id="edit{{$ak->kategori_id}}" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3>Edit Kategori {{$ak->kategori_nama}}</h3>
            <button class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
              <form method="post" action="{{route('crudkategori.update', $ak->kategori_id)}}">
                @csrf
                @method('PUT')
                ...
                <input type="text" name="kategori_nama" value="{{$ak->kategori_nama}}" class="form-control" id="nama" placeholder="Nama Kategori..." required>
                </label>
                </div>

                <div class="form-group">
                Deskripsi :
                <label class="input-group" for="desk">
                <div class="input-group-prepend">
                  <div class="input-group-text">
                  <i class="fa fa-comment"></i>
                  </div>
                </div>
                <textarea name="kategori_deskripsi" id="desk" class="form-control" placeholder="Deskripsi..." required>{{$ak->kategori_deskripsi}}</textarea>
                </label>
                </div>
    ...
    <!-- Modal Hapus Admin -->
    <div class="modal" id="hapus{{$ak->kategori_id}}" role="dialog" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h3>Hapus Kategori {{$ak->kategori_nama}}</h3>
            <button class="close" data-dismiss="modal">&times;</button>
          </div>
          <div class="modal-body">
            <form method="post" action="{{route('crudkategori.destroy', $ak->kategori_id)}}">
              @csrf
              @method('DELETE')
    ...
    <div class="modal" role="dialog" id="tambah" data-backdrop="static">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
              <form method="post" action="{{route('crudkategori.store')}}">
                @csrf
                @method('POST')
                <div class="form-group">
                Nama Kategori :
                ...
                <input type="text" name="kategori_nama" class="form-control" id="nama" placeholder="Nama Kategori..." required>
                </label>
                </div>
                ...
                <textarea name="kategori_deskripsi" id="desk" class="form-control" placeholder="Deskripsi..." required></textarea>
                </label>
                </div>
